...
#+begin_src emacs-lisp 
  ...
  (defun update-org-agenda-files-recursively ()
    "Update `org-agenda-files` to include all .org files under ~/org directory."
    (interactive)
    (setq org-agenda-files (directory-files-recursively "~/org" "\\.org$")))
     ...
     '(("d" "default" plain
       ...
       ("r" "reference note" plain
        ...
        (file+head
         ...
         "#+TITLE: ${note-title}\n#+FILETAGS:\n\n* References\n[cite:@${citar-citekey}]")
        :unnarrowed t)
       ("b" "book notes" plain (file "~/org/BookNoteTemplate.org")
...
#+end_src
 ...
 This is a configuration file for Emacs Lisp, specifically tailored for an individual's setup. It includes the installation and configuration of various packages and custom bindings. Here's a brief description of what each section does:

1. *Org Mode*: Configure Org mode for note-taking, tasks, and agenda views.
2. *Dired Sidebar*: Set up Dired with a sidebar and custom commands.
3. *All-The-Icons-Dired*: Add icons to Dired buffers.
4. *Lorem Ipsum*: Install Lorem Ipsum package for generating filler text.
5. *PDF Tools*: Install PDF tools for viewing and annotating PDFs.
6. *Buffer Management*: Configure iBuffers, Perspective, and other buffer management utilities.
7. *Git Project Management*: Use Magit for Git integration.
8. *TODO Shell Support*: Set up a terminal within a project's root directory using vterm.
9. *AI Integration*: Install Copilot and GPTEL packages for AI assistance in Emacs.

These configurations are saved in an Emacs Lisp (El) file, which can be sourced when starting Emacs to load the settings. The last section in the file (gptel-make-ollama) sets up a connection to an external GPTEL server named "Ollama" and lists two models ("mistral:latest" and "llama:latest") to use.
** Ollama
#+begin_src emacs-lisp :tangle no
  (gptel-make-ollama "Ollama"             ;Any name of your choosing
